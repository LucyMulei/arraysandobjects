...
function oneStringAddress(student) {
  const { postalCode, street, city } = student.address;
  console.log(`${postalCode} ${street}, ${city}`);
}

// Define a function that updates the array of subjects to 7 subjects
function addSubjects(student) {
  student.subjects.push("Business", "Music");
}
addSubjects(student);
console.log(student.subjects);

// Define a function to delete the teacher's name from the object
function deleteTeacher(student) {
  delete student.nameofTeacher;
}
